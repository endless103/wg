...
gulp.task('uglify', function (done) {
  ...
  return (
    gulp
      .src(['wwwroot/resources/scripts/**/*.js', BABEL_POLYFILL]) //src 폴더 아래의 모든 js 파일을
      .pipe(plumber(plumberOption))
      .pipe(babel())
      //.pipe(concat('all.js'))
      //.pipe(uglify()) //minify 해서
      .pipe(gulp.dest('wwwroot/resources/scripts/dist'))
  ); //dist 폴더에 저장
});
...
gulp.task('sassPC', function () {
  return gulp
    .src('wwwroot/resources/scss/**/*.scss')
    .pipe(plumber(plumberOption))
    .pipe(
      sourcemaps.init({
        loadMaps: true,
    ...
    .pipe(gulp.dest('wwwroot/resources/styles'))
...
gulp.task('buildPC', function () {
  return gulp
    ...
    .pipe(gulp.dest('wwwroot/resources/styles/dist'))
    .pipe(browserSync.reload({ stream: true }))
    .on('end', function () {
